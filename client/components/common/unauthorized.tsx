  ...
  const handleSignIn = () =>{
    ...
    Axios.post(`/api/user/sign-in`, user).then(({data}) => {
      ...
      if (err.response?.status == 409)
        handleAddMessage({ type: 'fail', title: 'Đăng nhập thất bại', content: 'Đăng nhập ' + userName + ' thất bại' });
      else
        handleAddMessage({ type: 'fail', title: 'Đăng nhập thất bại', content: 'Đăng nhập của bạn không được hoàn thành' });
    })
      .finally(() => setLoading(false))
  }
  return (
    <div className="w-full h-screen fixed top-0 bg-gray-800">
      <div id="container">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1278 1080">
          ...
          <g id="Layer_2" data-name="Layer 2">
            ...
            <g id="Layer_1-2" data-name="Layer 1">
              <g id="earth" className="hover:opacity-50 hover:cursor-pointer" onClick={() => window.location.assign('/')}>
        ...
        <h1 className="text-[30px]">FORBIDDEN <br /> ADMIN ONLY</h1>
        <div className="w-full max-w-xs mx-auto mb-16">
          <form className="bg-gray-800 shadow-md rounded px-8 pt-6 pb-8">
            <div className="mb-4">
              ...
              <input onChange={(e) => setUsername(e.target.value)} className="appearance-none bg-gray-700 shadow border rounded w-full py-2 px-3 text-white leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="Username" />
            </div>
            <div className="mb-6">
              <label className="block text-gray-300 text-sm font-bold mb-2">
                Password
              </label>
              <input onChange={(e) => setPassword(e.target.value)} className="shadow appearance-none bg-gray-700 border rounded w-full py-2 px-3 text-white mb-3 leading-tight focus:outline-none focus:shadow-outline" id="password" type="password" placeholder="******************" />
            </div>
            <div className="flex items-center justify-between">
              <div className="mx-auto hover:opacity-70">
              <LoadingButton
                className="mx-auto"
                size='medium'
                style={{ background: '#538591' }}
                variant='contained'
